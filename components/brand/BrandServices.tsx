      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mt-14 grid grid-cols-1 md:grid-cols-2 gap-6">
          {brand.specialties.map((s, i) => (
            <article
              key={i}
              className="border border-white/10 hover:border-bone transition-colors p-8"
            >
              <p className="text-[10px] uppercase tracking-[0.3em] text-graphite">
                Capability {String(i + 1).padStart(2, "0")}
              </p>
              <h3 className="mt-3 font-display text-2xl md:text-3xl text-bone tracking-wide leading-tight">
                {s.title}
              </h3>
              <p className="mt-4 text-bone/85 max-w-[60ch]">{s.copy}</p>
            </article>
          ))}
        </div>
      </div>
